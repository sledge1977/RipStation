...
class ParsingTests(unittest.TestCase):
    ...
    def test_range_formatting(self):
        self.assertEqual(
            rip_station.format_number_ranges([1, 2, 3, 6, 8, 7]), "1-3,6,8-7"
        )

    def test_fetch_drive_info_raises_instead_of_exiting(self):
        with patch(
            "rip_station.subprocess.run", side_effect=FileNotFoundError("missing")
        ):
    ...
    def test_track_scan_falls_back_to_disc_id(self):
        ...
        with patch(
            "rip_station.subprocess.run", side_effect=[first_error, result]
        ) as run:
            tracks = rip_station.get_disc_tracks(drive, 60)
    ...
    def test_non_tty_menu_accepts_multi_digit_drive_id(self):
        ...
        with (
            patch.object(rip_station.sys, "stdin", stream),
...
class ResponsiveUiTests(unittest.TestCase):
    def setUp(self):
        self.drives = [
            rip_station.Drive(
                0, "/dev/sr0", "SERIES_DISC_WITH_A_LONG_NAME", "HL-DT-ST Blu-ray"
            ),
        ...
        for width in (20, 40, 67, 68, 90, 109, 110, 140):
            for height in (8, 12, 24):
                with self.subTest(width=width, height=height):
                    ...
                    self.assertTrue(
                        all(rip_station.display_width(line) <= width for line in lines)
                    )
    ...
    def test_series_selection_uses_responsive_layouts(self):
        ...
        for width in (20, 47, 48, 75, 76, 100):
            with self.subTest(width=width):
                ...
                self.assertTrue(
                    all(
                        rip_station.display_width(line) <= width
...
class SeriesDetectionTests(unittest.TestCase):
    ...
    def test_incomplete_markers_fall_back_to_track_id(self):
        ...
        self.assertEqual(
            [t["id"] for t in rip_station.order_episode_tracks(tracks)], [3, 7]
        )
...
class JobTests(unittest.TestCase):
    ...
    def test_prompt_defaults_to_next_existing_episode(self):
        ...
        self.assertEqual(
            [os.path.basename(job[1]) for job in jobs],
            [
                "Show.S01E05.mkv",
                "Show.S01E06.mkv",
            ],
    ...
    def test_worker_reports_unexpected_setup_error(self):
        ...
        with (
            patch("rip_station.os.makedirs", side_effect=OSError("read only")),
            redirect_stdout(io.StringIO()),
        ):
            rip_station.rip_jobs_worker(
                drive, [(1, "/video/Show.S01E01.mkv", "title.mkv")]
            )
    ...
    def test_worker_uses_fixed_source_and_isolated_staging_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch("rip_station.subprocess.Popen", side_effect=start_process),
                patch(
                    "rip_station.subprocess.run",
    ...
    def test_worker_terminates_and_kills_hung_process_before_release(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            drive.busy = True
            with rip_station.job_state_lock:
                rip_station.reserved_outputs.add(
                    rip_station.canonical_output_path(output)
                )
            with (
                patch("rip_station.subprocess.Popen", return_value=process),
                redirect_stdout(io.StringIO()),
            ):
                rip_station.rip_jobs_worker(
    ...
    def test_rescan_does_not_mutate_busy_drive(self):
        ...
        try:
            with (
                patch(
                    "rip_station.fetch_drive_info",
                    return_value=[(12, "New drive", "NEW_DISC", "/dev/sr2")],
                ),
                patch("rip_station.time.sleep"),
                redirect_stdout(io.StringIO()),
            ):
    ...
    def test_rescan_resets_idle_drive_even_when_volume_label_is_unchanged(self):
        ...
        try:
            with (
                patch(
                    "rip_station.fetch_drive_info",
                    return_value=[(1, "Updated drive", "SAME_LABEL", "/dev/sr0")],
                ),
                patch("rip_station.time.sleep"),
                redirect_stdout(io.StringIO()),
            ):
    ...
    def test_failed_thread_start_releases_drive_and_outputs(self):
        ...
        with patch(
            "rip_station.threading.Thread", side_effect=RuntimeError("no thread")
        ):
...
class NewFeatureTests(unittest.TestCase):
    def test_next_episode_detects_various_separators(self):
        """Ensure spaces, dashes and underscores before S01E05 are detected."""
        with tempfile.TemporaryDirectory() as temp_dir:
            season_dir = os.path.join(temp_dir, "Show", "Season 01")
            os.makedirs(season_dir)
            ...
            with patch.object(rip_station, "BASE_OUTPUT_DIR", temp_dir):
                self.assertEqual(rip_station.next_episode_number("Show", 1), 7)

    def test_clean_label_for_suggestion(self):
        self.assertEqual(
            rip_station.clean_label_for_suggestion("THE_MATRIX_1999"), "The Matrix 1999"
        )
        self.assertEqual(
            rip_station.clean_label_for_suggestion("BREAKING_BAD_S01D02"),
            "Breaking Bad",
        )
        self.assertEqual(rip_station.clean_label_for_suggestion("DISC_1"), "")
        self.assertEqual(rip_station.clean_label_for_suggestion("DVD_VIDEO"), "")
        self.assertEqual(rip_station.clean_label_for_suggestion(""), "")

    def test_eject_drive_windows_powershell(self):
        with (
            patch("rip_station.os.name", "nt"),
            patch(
                "rip_station.subprocess.run", return_value=SimpleNamespace(returncode=0)
            ) as run,
            redirect_stdout(io.StringIO()),
        ):
            rip_station.eject_drive("D:")
    ...
    def test_cancel_drive_rip(self):
        ...
        drive.active_process = fake_proc

        success = rip_station.cancel_drive_rip(drive)
        self.assertTrue(success)
        self.assertTrue(drive.cancel_requested)

    def test_stop_all_workers(self):
        d1 = rip_station.Drive(1, "/dev/sr1", "D1", "Drive1")
        d2 = rip_station.Drive(2, "/dev/sr2", "D2", "Drive2")
        d1.busy = True
        d2.busy = False

        rip_station.stop_all_workers([d1, d2])
        self.assertTrue(d1.cancel_requested)
        self.assertFalse(d2.cancel_requested)

    def test_parse_arguments(self):
        cfg = rip_station.parse_arguments(
